  ...
  return (
    <div className="bg-cyber-card border border-cyber-border rounded-xl p-6 space-y-6">
      ...
      <div className="border-b border-cyber-border pb-4 flex justify-between items-center">
        <div>
          <h2 className="text-base font-extrabold font-sans text-slate-100">
        ...
        <span className="text-[10px] font-mono px-2.5 py-1 rounded bg-cyber-dark text-cyan-400 border border-cyber-border font-bold">
          POST /api/report
        </span>
      </div>
        ...
        <div className="space-y-2">
          {THREAT_PRESETS.map((preset) => (
            <button
              ...
              className="w-full text-left p-3 rounded-lg bg-cyber-dark hover:bg-cyber-card-hover border border-cyber-border text-slate-300 transition flex justify-between items-center text-xs font-mono group"
            >
              <div className="truncate pr-2">
                <span className="text-cyan-400 font-bold mr-2">[{preset.category}]</span>
                <span className="text-slate-200 group-hover:text-white transition">{preset.label}</span>
              </div>
              <span className="text-[11px] text-slate-500 group-hover:text-cyan-400 shrink-0 font-bold">Load Indicator →</span>
            </button>
          ))}
        </div>
      </div>
      ...
      {sanitizationPreview && (
        <div className="p-4 bg-cyber-dark border border-cyber-border rounded-lg space-y-2 font-mono text-xs">
          <div className="flex justify-between items-center text-[11px] text-slate-400 border-b border-cyber-border pb-2">
          ...
          <div>
            ...
            <p className="text-emerald-300 break-all font-bold bg-cyber-card p-2 rounded border border-cyber-border">
          ...
          {!sanitizationPreview.isClean && (
            <div>
              <span className="text-[10px] text-slate-500 uppercase block mb-1">Stripped Query Parameters</span>
              <div className="flex flex-wrap gap-1">
                {sanitizationPreview.strippedParams.map((param, idx) => (
                  <span key={idx} className="text-[10px] bg-rose-500/10 text-rose-400 border border-rose-500/30 px-2 py-0.5 rounded">
                    {param}
                  </span>
                ))}
              </div>
      ...
      {result && result.type === 'success' && (
        <div className="p-4 bg-cyber-dark border border-emerald-500/40 rounded-lg font-mono text-xs text-emerald-400 space-y-2">
          ...
          {result.threat && (
            <div className="pt-2 text-slate-300 text-xs grid grid-cols-2 gap-2 border-t border-cyber-border">
              <div>Risk Score: <strong className="text-slate-100">{result.threat.score}/100</strong></div>
              <div>Category: <strong className="text-cyan-400">{result.threat.type}</strong></div>
            </div>
          )}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-4 font-sans text-xs">
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-3 bg-cyber-dark border border-cyber-border rounded-lg text-slate-100 placeholder-slate-500 outline-none focus:border-cyan-500 transition font-mono text-xs"
            disabled={loading}
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full p-3 bg-cyber-dark border border-cyber-border rounded-lg text-slate-100 placeholder-slate-500 outline-none focus:border-cyan-500 transition text-xs"
            disabled={loading}
          ></textarea>
        </div>
        ...
        <button
          ...
          className="w-full bg-cyan-500 hover:bg-cyan-400 text-cyber-dark font-extrabold py-3 px-4 rounded-lg transition duration-200 disabled:opacity-50 disabled:cursor-not-allowed font-mono uppercase tracking-wider text-xs shadow-md shadow-cyan-500/20"
        >
