...
export default function IncidentStreamTable({ extraThreats = [] }) {
  ...
  // Combine default incident dataset with user-submitted threats (ensuring unique IDs & realistic fields)
  const allIncidents = useMemo(() => {
    const existingIds = new Set(RECENT_INCIDENTS.map(i => i.id));
    
    const customFormatted = extraThreats.map((t, idx) => {
      let uniqueId = t.id;
      if (!uniqueId || existingIds.has(uniqueId)) {
        uniqueId = `SUB-${8000 + idx + 1}`;
      }

      const score = t.score || Math.floor(Math.random() * 30) + 65;
      const confidence = (88 + (score * 0.11)).toFixed(1) + '%';
      
      const typeStr = t.type || (t.indicator?.includes('http') ? 'Phishing Domain' : t.indicator?.includes('@') ? 'Typosquat Domain' : 'Scanner Host');
      
      const targetStr = typeStr.includes('Phishing') ? 'Auth Portal (443)' : typeStr.includes('Scanner') ? 'SSH Daemon (22)' : 'Mail Ingress (25)';
      const locationStr = 'Local Ingestion Node';

      return {
        id: uniqueId,
        severity: score >= 85 ? 'Critical' : score >= 60 ? 'High' : score >= 35 ? 'Medium' : 'Low',
        score: score,
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="card-3d rounded-2xl p-6 max-w-lg w-full space-y-4 font-sans text-xs shadow-2xl border border-white/20">
            ...
            <div className="pt-2 flex justify-end gap-2">
              <button
                onClick={() => setSelectedIncident(null)}
                className="btn-3d-secondary px-4 py-2 rounded-xl text-xs font-medium"
              >
                Close Window
              </button>
            </div>
